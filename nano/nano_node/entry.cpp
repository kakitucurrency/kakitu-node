...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("debug_profile_process", "Profile active blocks processing (only for kshs_dev_network)")
		("debug_profile_votes", "Profile votes processing (only for kshs_dev_network)")
		("debug_profile_frontiers_confirmation", "Profile frontiers confirmation speed (only for kshs_dev_network)")
	...
	if (ec == nano::error_cli::unknown_command)
	{
		...
		else if (vm.count ("debug_sys_logging"))
		{
			...
			{
				std::cerr << "The event log requires the HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Kakitu\\Kakitu registry entry, run again as administator to create it.\n";
				return 1;
			}
#endif
			auto inactive_node = nano::default_inactive_node (data_path, vm);
