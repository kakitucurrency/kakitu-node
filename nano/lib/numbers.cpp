...
void nano::public_key::encode_account (std::string & destination_a) const
{
	...
	destination_a.append ("_shsk"); // kshs_
...
bool nano::public_key::decode_account (std::string const & source_a)
{
	...
	if (!error)
	{
		...
		auto kshs_prefix (source_a[0] == 'k' && source_a[1] == 's' && source_a[2] == 'h' && source_a[3] == 's' && (source_a[4] == '_' || source_a[4] == '-'));
		auto node_id_prefix = (source_a[0] == 'n' && source_a[1] == 'o' && source_a[2] == 'd' && source_a[3] == 'e' && source_a[4] == '_');
		error = (xrb_prefix && source_a.size () != 64) || (kshs_prefix && source_a.size () != 65);
		if (!error)
		{
			if (xrb_prefix || kshs_prefix || node_id_prefix)
			{
